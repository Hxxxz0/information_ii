...
export const Window: React.FC<WindowProps> = ({ title, isOpen, onClose, children, variant = 'default' }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm p-4 lg:p-6 animate-in fade-in zoom-in duration-200">
      <div
        className={`w-full max-w-6xl h-[85vh] lg:h-[85vh] flex flex-col overflow-hidden relative transition-all
          ${isImmersive
            ? 'bg-black border border-cyan-500/50 shadow-[0_0_50px_rgba(6,182,212,0.3)] rounded-xl'
            : 'bg-white rounded-xl lg:rounded-[2rem] shadow-2xl border-4 border-white'
          }`}
